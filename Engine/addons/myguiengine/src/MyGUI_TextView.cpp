...
Loop_Re:
				...
				&& !roll_back.empty())
			{
				...
				// --------------- genesis code ---------
				bool need_draw = (FontCodeType::Space != character && _text.begin() != index);//[2014/2/18 zhongdaohuan] 空格可以不画，非空格就不可以了。
				if (need_draw)
				{
					line_info.count = count;
					mLength += count;
				}
				else //空格在行尾可以不画，但占一个位置
				{
					++count;
					line_info.count = count;
					mLength += count;
				...
				// отменяем откат
				roll_back.clear();
				if (need_draw)//[2012/4/27 zhongdaohuan] 空格可以不画，非空格就不可以了。	
				{
					character = *index;
					goto Loop_Re;
				}
	...
	size_t TextView::getCursorPosition(const IntPoint& _value)
	{
		...
		for (VectorLineInfo::const_iterator line = mLineInfo.begin(); line != mLineInfo.end(); ++line)
		{
			...
			{
				top += height;
				//result += line->count + 1;//old code
				result += line->count; //[2014/2/18]
			}
		}
		return result;
	}
	...
	IntPoint TextView::getCursorPoint(size_t _position)
	{
		...
		for (VectorLineInfo::const_iterator line = mLineInfo.begin(); line != mLineInfo.end(); ++line)
		{
			...
			//position += line->count + 1;//old code
			position += line->count;//[]2014/2/28]
			top += mFontHeight;
		}
